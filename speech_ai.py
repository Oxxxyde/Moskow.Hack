...
class Speech_AI:
    ...
    def work(self):
        ...
        with self._microphone as source:
            self._recognizer.adjust_for_ambient_noise(source, duration = 30)
            ...
            while True:
                ...
                try:
                    ...
                    if(((statement.find("youtube")!=-1) or (statement.find("ютуб")!=-1) or (statement.find("you tube")!=-1)) and (statement.find("смотреть")!=-1)):
                        ...
                        statement=statement.replace('смотреть', '')
                        statement=statement.strip()
                        self.openurl('http://www.youtube.com/results?search_query=' + statement, 'Поиск на YouTube')
                    ...
                    if((statement.find("спать")!=-1) or (statement.find("слиип")!=-1)):
                        answer = "Выполняю, сэр!"
                        self.say(str(answer))
                    ...
                    print("Вас плохо слышно! Повторите еще раз.")
                except sr.RequestError as e:
                    print("Код ошибки: 2 {0}".format(e))
